...
export function FaceCamera({
  ...
  if (state === "INSTRUCTIONS") {
    return (
      <div className="w-full overflow-hidden rounded-3xl bg-zinc-950 text-white shadow-2xl">
        <div className="flex min-h-[360px] flex-col justify-between p-5 sm:min-h-[420px] sm:p-8">
          <div className="text-center">
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-red-600/15 text-red-300 ring-1 ring-red-500/20">
              <ShieldCheck className="h-8 w-8" />
            </div>
            <p className="text-[10px] font-black uppercase tracking-[0.24em] text-red-300">SafeEPI</p>
            <h3 className="mt-2 text-2xl font-black uppercase tracking-tight text-white sm:text-3xl">{title}</h3>
            <p className="mx-auto mt-3 max-w-sm text-sm leading-relaxed text-zinc-300">
              Centralize o rosto em boa luz. A proxima tela abre a camera.
            </p>
          </div>
          ...
          <div className="mt-6 flex flex-col gap-3">
            <button
              ...
              className="min-h-[56px] w-full rounded-2xl bg-red-600 px-5 py-4 text-sm font-black uppercase tracking-[0.16em] text-white shadow-xl shadow-red-950/30 transition hover:bg-red-700 active:scale-[0.99]"
            >
              Iniciar camera
            </button>
            <button
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-3xl bg-zinc-950 text-white shadow-2xl">
      ...
      <div className="relative grid gap-3 p-3 sm:gap-5 sm:p-6 lg:grid-cols-[minmax(360px,1fr)_340px] lg:p-7">
        <section className="flex flex-col items-center">
          <div className="mb-2 flex w-full items-center justify-between gap-3 sm:mb-4">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.24em] text-red-300">{modeLabel}</p>
              <h3 className="text-sm font-black uppercase tracking-tight text-white sm:text-lg">{title}</h3>
            </div>
            <button
              type="button"
              onClick={handleCancel}
          ...
          <div className="relative aspect-square w-full max-w-[min(86vw,390px)] sm:max-w-[440px] lg:max-w-[520px]">
        ...
        <section className="flex min-w-0 flex-col justify-between gap-3 rounded-3xl border border-white/10 bg-white/[0.06] p-3 sm:p-5">
          <div>
            <div className="mb-3 flex items-center gap-3">
              <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl sm:h-12 sm:w-12 ${state === "APPROVED" ? "bg-emerald-400/15 text-emerald-300" : isEvidenceMode ? "bg-amber-400/15 text-amber-300" : "bg-red-500/15 text-red-300"}`}>
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm font-semibold leading-snug text-white sm:text-lg">{instruction}</p>
              </div>
            </div>

            {isEvidenceMode && (
              <div className="mb-3 rounded-2xl border border-amber-300/25 bg-amber-300/10 p-3 text-xs leading-relaxed text-amber-100 sm:text-sm">
